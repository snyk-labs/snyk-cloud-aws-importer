...
def main(
    ...
    if not (SNYK_TOKEN and AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY):
        ...
        print(stylize("environment variables `SNYK_TOKEN`, `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` must be set", STYLE_ERR))
    ...
    if type(config) is yaml.YAMLError:
        ...
        print(stylize(f"could not read config file, please check your config - {str(config)}", STYLE_ERR))
